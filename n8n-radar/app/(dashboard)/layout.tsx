  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "var(--bg-canvas)" }}>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-4 py-3"
        style={{ background: "var(--bg-sidebar)", boxShadow: "0 1px 0 0 var(--border)" }}>
        <Wordmark />
        <button onClick={() => setMobileOpen((v) => !v)}
          className="p-2 rounded-xl" style={{ background: "#0d0d16" }}>
          {mobileOpen ? <X size={16} color="#a1a1aa" /> : <Menu size={16} color="#a1a1aa" />}
        </button>
      </div>

      {/* ── Mobile overlay ── */}
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-20" style={{ background: "rgba(0,0,0,0.6)" }}
          onClick={() => setMobileOpen(false)} />
      )}

      {/* ── Sidebar ── */}
      <aside className={`flex flex-col w-60 shrink-0 fixed md:relative top-0 bottom-0 z-30 transition-transform md:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
        style={{
        ...
        <div className="px-4 pt-5 pb-4"
          ...
          <Wordmark tagline="Automation Grid Monitor" />
        </div>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  color: active ? "#67e8f9" : "#5a5a68",
                  boxShadow: active ? "0 0 0 1px rgba(34,211,238,0.35)" : "none",
              ...
              >
                ...
                <Icon size={14} color={active ? "#67e8f9" : "currentColor"} />
                {label}
        ...
        <div className="px-3 pb-4 flex flex-col gap-2"
          ...
          <p className="text-[10px] px-1" style={{ color: "#2e2e38" }}>
            PulseGrid · {summary?.total ?? 0} workflows
          </p>
        </div>
      </aside>

      {/* Main */}
      <main className="flex-1 overflow-auto pt-14 md:pt-0" style={{ background: "var(--bg-canvas)" }}>
        {children}
      </main>
    </div>
  )
